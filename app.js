...
app.post('/getuserDetails', function(req, res) {
    ...
    var usertest = model.auth.findOneAndUpdate({ 'username': req.body.user }, { $set: { 'lastlogin': new Date() } }, { upsert: true }, (err, user) => {
        if (!err) {
            console.log('event fired no error');
            res.send(user);
        } else {
            console.log('event fired with error error');
            res.status(404).send("failure");
        }
    });
    // 	var userData = model.user.findOne({ 'data.username' :  req.body.user },
...
app.post('/getuserDetailss', function(req, res) {
    console.log('inside user details');
    console.log(req.body);
    console.log(req.body.username);


    var usertest = model.auth.findOne({ 'username': req.body.username }, (err, authUser) => {
        if (!err) {
            console.log('user found');
            console.log(authUser);

            model.user.findOne({ 'data.username': req.body.username }, (error, user) => {
                if (!error) {
                    console.log(user)
                    console.log(typeof(user));
                    Object.defineProperty(user, "lastLogin", { value: authUser.lastlogin });

                    // user.push({ "lastLogin": authUser.lastlogin });
                    console.log(user);

                    res.json({ 'user': user, 'lastLogin': authUser.lastlogin });
                } else {
                    console.log('user found with error error');
                    res.status(404).send("failure");
                }
